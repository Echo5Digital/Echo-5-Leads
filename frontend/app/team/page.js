...
// Rules: which roles each role can assign
const ASSIGNABLE_ROLES = {
  super_admin: ['super_admin', 'client_admin', 'ceo', 'cfo', 'manager', 'staff', 'member'],
  client_admin: ['member'],
  ceo: ['client_admin', 'ceo', 'cfo', 'manager', 'staff', 'member'],
  cfo: ['client_admin', 'ceo', 'cfo', 'manager', 'staff', 'member'],
...
export default function TeamPage() {
  ...
  const canAddUsers = assignableRoles.length > 0;
  ...
  function openCreateModal() {
    ...
    setFormData({
      ...
      email: '',
      password: '',
      role: assignableRoles[0] || ROLES.MEMBER,
      active: true
    });
    setShowModal(true);
  }

  function openEditModal(teamUser) {
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {users.length === 0 ? (
                <tr>
                  <td colSpan={canAddUsers ? 5 : 4} className="px-6 py-8 text-center text-gray-500">
                    No team members found.{canAddUsers ? ' Click "Add Team Member" to get started.' : ''}
                  </td>
                </tr>
              ) : (
                users.map((teamUser) => (
                  <tr key={teamUser._id} className={!teamUser.active ? 'bg-gray-50 opacity-60' : ''}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${ROLE_BADGE_COLORS[teamUser.role] || 'bg-gray-100 text-gray-700'}`}>
                        {ROLE_LABELS[teamUser.role] || teamUser.role}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${teamUser.active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
